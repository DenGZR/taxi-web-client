...
export function show (value: number = 1): Action {
  return {
    type: SHOW,
    payload: value
  }
...
const ACTION_HANDLERS = {
  ...
  [SHOW]: (state: number, action: {payload: number}): number => state - action.payload
}
...
// export default function showReducer (state: number = initialState, action: Action): number {
//   const handler = ACTION_HANDLERS[action.type]
//
//   return handler ? handler(state, action) : state
// }
